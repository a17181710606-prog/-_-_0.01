...
interface State {
  ...
  // 数据加载状态
  dataReady: boolean      // 首次从后端拉取是否完成
  usingSeed: boolean      // 当前展示的是否为本地种子数据（未接后端）
...
// 本地时区的 "YYYY-MM-DD HH:mm"，与 todayStr() 前缀匹配，供工作台统计今日出入库
...
export const useStore = create<Store>()(
  immer((set, get) => ({
    ...
    // ── 数据加载 ─────────────────────────────────────────
    // 已配置 Supabase：以后端数据为准（含空库，供导入真实设备）。
    // 未配置：保留本地种子数据，方便无 key 时预览。
    hydrate: async () => {
      if (!db.dbEnabled()) {
        set(s => { s.dataReady = true; s.usingSeed = true })
        return
      }
      try {
        const [equipment, movements] = await Promise.all([
          db.fetchEquipment(),
          db.fetchMovements(),
        ])
        set(s => {
        ...
        set(s => {
          s.dataReady = true
          s.usingSeed = true
          s.toasts.push({ id: Date.now().toString(), msg: '设备数据加载失败，已临时使用本地示例数据', type: 'err' })
        })
      }
    },
    ...
    submitCart: (projectName, days) => {
      const order: RentalOrder = {
        id: `ORD-${Date.now()}`,
        projectName,
        items: get().cart.map(i => ({ equipmentId: i.equipmentId, qty: i.qty })),
        days,
      ...
      if (db.dbEnabled()) {
        db.insertOrder(order).catch(() => get().toast('申请已记录，但同步到服务器失败', 'err'))
      }
    },
    ...
    // equipment CRUD —— 后端优先，成功后再更新本地状态，写入失败给出提示且不改动本地
    addEquipment: async (e) => {
      ...
      try {
        const row = await db.insertEquipment(e)
        set(s => {
          s.equipment.push(row)
          s.toasts.push({ id: Date.now().toString(), msg: '设备已添加', type: 'ok' })
        })
      } catch { get().toast('添加失败，请检查网络或登录状态', 'err') }
    },
    updateEquipment: async (id, patch) => {
      if (db.dbEnabled()) {
        try { await db.updateEquipment(id, patch) }
        catch { get().toast('更新失败，请检查网络或登录状态', 'err'); return }
      }
      set(s => {
    ...
    deleteEquipment: async (ids) => {
      if (db.dbEnabled()) {
        try { await db.deleteEquipment(ids) }
        catch { get().toast('删除失败，请检查网络或登录状态', 'err'); return }
      }
      set(s => {
    ...
    updateEquipmentStatus: async (id, st) => {
      if (db.dbEnabled()) {
        try { await db.updateEquipment(id, { st }) }
        catch { get().toast('状态更新失败', 'err'); return }
      }
      set(s => {
        const eq = s.equipment.find(e => e.id === id)
        if (eq) eq.st = st
    ...
    batchSetStatus: async (ids, st) => {
      if (db.dbEnabled()) {
        try { await db.updateEquipmentMany(ids, { st }) }
        catch { get().toast('批量修改失败，请检查网络或登录状态', 'err'); return }
      }
      set(s => {
    ...
    importEquipment: async (rows) => {
      ...
      try {
        const inserted = await db.insertEquipmentMany(rows)
        set(s => {
          s.equipment.push(...inserted)
          s.importModalOpen = false
          s.toasts.push({ id: Date.now().toString(), msg: `已导入 ${inserted.length} 件设备`, type: 'ok' })
        })
      } catch { get().toast('导入失败，请检查网络或登录状态', 'err') }
    },

    // records —— 生成完整记录后本地入列并写库（写库失败给出提示）
    addMovement: (m) => {
      ...
      set(s => { s.movements.unshift(record) })
      if (db.dbEnabled()) {
        db.insertMovement(record).catch(() => get().toast('记录已保存到本地，但同步到服务器失败', 'err'))
      }
    },
    ...
    // inquiries
    addInquiry: (serviceId, contactName) => {
      ...
      if (db.dbEnabled()) {
        db.insertInquiry(inquiry).catch(() => get().toast('咨询已记录，但同步到服务器失败', 'err'))
      }
    },
